...
def main():
    ...
    # p_train, h_train, train_labels = load_data(snli("train"), False)
    ...
    with open("bowModel(2).pkl", "wb") as modelFile:
        pickle.dump(model, modelFile)
    

    print("Testing the model")
    p_test, h_test, test_labels = load_data(snli("test"), False)
    p_test = list(map(lambda x: pre_process(x), p_test))
    h_test = list(map(lambda x: pre_process(x), h_test))

    correct = 0
    predictions = []
    label_types = ["entailment", "contradiction"]
